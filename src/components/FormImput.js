...
class FormInput extends React.Component {
  ...
  render() {
    const { title, date, id, status, onClickDelete, checked, onCheckChange } = this.props;
    return (
      <div className={`bg-white flex ${status === "1" ? 'bg-light' : (status === "2" ? 'bg-warning' : 'bg-white')}`}>
        <div className="flex fix">
          <input type="checkbox" value={id}
            checked={checked} onChange={onCheckChange}
          />
        ...
        <div className="remove-edit">
          <button type="button" className="btn-icon" disabled ={!checked}
